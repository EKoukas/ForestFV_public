...
void m4_facevalues(struct RUN *run,struct BRANCH * crnt,int in,int ifc,int bc_temp,int imode) {
  ...
  struct SOL * r_sol;
  struct LEAF * temp_el;
  double press,avfsum,avf_dif,avf_per;
  double avf_temp[10];
  double avf_c_temp[10];

  double amin_100 = AMIN/100.0;
  ...
  r_sol->r=0.0;
	for(v=0;v<eqtypn[0];++v){
	  r_sol->r     += temp_el->S[v];
	  r_sol->ra[v]  = temp_el->S[v];
	  r_sol->vec[v] = temp_el->S[v];
	}
  ...
  v = eqtypi[1];
	r_sol->u = temp_el->S[v+0] / r_sol->r;
	r_sol->v = temp_el->S[v+1] / r_sol->r;
	r_sol->w = temp_el->S[v+2] / r_sol->r;

  r_sol->vec[v+0] = temp_el->S[v+0];
  r_sol->vec[v+1] = temp_el->S[v+1];
  r_sol->vec[v+2] = temp_el->S[v+2];
  ...
  for(iv=0;iv<eqtypn[3];++iv){ 
    avf_temp[iv]   = temp_el->S[iv+eqtypi[3]]; 
    avf_c_temp[iv] = temp_el->S[iv+eqtypi[3]]; 
  }

  avfsum=0.0; 
  for(v=0;v<eqtypn[3];++v){
    if      (avf_temp[v]  <=      amin_100)   { avf_temp[v]   =      amin_100;  } 
    else if (avf_temp[v]  >=(1.0-(amin_100))) { avf_temp[v]   = 1.0-(amin_100); }
    
    if      (avf_c_temp[v]<=      amin_100)   { avf_c_temp[v] =      amin_100;  } 
    else if (avf_c_temp[v]>=(1.0-(amin_100))) { avf_c_temp[v] = 1.0-(amin_100); }
    avfsum   += avf_temp[v]; 
  }  
  
  if (avfsum>(1.0-(amin_100))) { 
    avf_dif = avfsum - (1.0-(amin_100));  
  ...
  // are
	for(iv=0;iv<eqtypn[4];++iv){
	  r_sol->are[iv]           = temp_el->S[iv+eqtypi[4]];
    r_sol->vec[eqtypi[4]+iv] = temp_el->S[iv+eqtypi[4]];
	}
  ...
  for (v=0;v<eqtypn[4];++v) {   
    ...
    press = ((MATERGAMA[v]-1.0)*(r_sol->are[v])/avf_temp[v]) - MATERGAMA[v]*MATERPINF[v];  // Stiffened-gas / ideal gas

    r_sol->c2Y[v] = (avf_temp[v]/r_sol->r)*MATERGAMA[v]*(press+MATERPINF[v]);

    r_sol->st[0][0] += -avf_temp[v]*press;
    r_sol->st[1][1] += -avf_temp[v]*press;
    r_sol->st[2][2] += -avf_temp[v]*press;
