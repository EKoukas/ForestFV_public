...
void m2_hllc(struct RUN *run,struct BRANCH * crnt,int in,int ifc) {
  ...
  m2_wave_speeds(run,crnt,ifc,&S_S,&SL,&SR,&uL,&vL,&wL,&s11L,
                                           &uR,&vR,&wR,&s11R);
  ...
  if (( (SL<=0.0)&&(0.0<=S_S) ) && (flags_if==0)) { // Left star state  
    flags_if=2;
    m2_star_region(run,crnt,run->sol_L,ifc,1,S_S,SL,SR,uL,vL,wL,uR,vR,wR);
    m2_flux(run->sol_L);          
    for(iv=0;iv<NEQ;++iv){
  ...
  if (( (S_S<=0.0)&&(0.0<=SR) ) && (flags_if==0)) { // Right star state
    flags_if=3;
    m2_star_region(run,crnt,run->sol_R,ifc,2,S_S,SL,SR,uL,vL,wL,uR,vR,wR);
    m2_flux(run->sol_R);          
    for(iv=0;iv<NEQ;++iv){
