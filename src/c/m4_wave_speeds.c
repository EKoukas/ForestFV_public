...
void m4_wave_speeds(struct RUN *run, struct BRANCH * crnt,int ifc,double* u_S,double* SL, double* SR,
  ...
  nx=crnt_cl->nx[ifc]; mx=crnt_cl->nxt1[ifc]; lx=crnt_cl->nxt2[ifc];
  ny=crnt_cl->ny[ifc]; my=crnt_cl->nyt1[ifc]; ly=crnt_cl->nyt2[ifc];
  nz=crnt_cl->nz[ifc]; mz=crnt_cl->nzt1[ifc]; lz=crnt_cl->nzt2[ifc];
  ...
  *SL = min( (*uL-cL) , (*uR-cR) );   // Left  wave speed 
  ...
  rotate_tensor(tensor_temp,run->sol_L->st,crnt_cl->nx[ifc]  ,crnt_cl->ny[ifc]  ,crnt_cl->nz[ifc],
                                         crnt_cl->nxt1[ifc],crnt_cl->nyt1[ifc],crnt_cl->nzt1[ifc],
                                         crnt_cl->nxt2[ifc],crnt_cl->nyt2[ifc],crnt_cl->nzt2[ifc],
  ...
  *s13L = tensor_temp[2];
  ...
  rotate_tensor(tensor_temp,run->sol_R->st,crnt_cl->nx[ifc]  ,crnt_cl->ny[ifc]  ,crnt_cl->nz[ifc],
                                          crnt_cl->nxt1[ifc],crnt_cl->nyt1[ifc],crnt_cl->nzt1[ifc],
                                          crnt_cl->nxt2[ifc],crnt_cl->nyt2[ifc],crnt_cl->nzt2[ifc],
                                          VERBOSE,run->con->rank);
  *s11R = tensor_temp[0];
  ...
  *u_S=((run->sol_L->r*pow ((*uL),2.0)- (*s11L)) - (run->sol_R->r*pow( (*uR),2.0)- (*s11R)) - ( (*SL)*ruL) + ( (*SR)*ruR)) / 
  ...
  if ( (isnan(*u_S)==1) || (isnan(*SR)==1) || (isnan(*SL)==1) ) {
    ...
    for (ind=0;ind<crnt->nlnd;ind++){
      rx+=crnt_cl->nd[ind].x/((double)crnt->nlnd);
      ry+=crnt_cl->nd[ind].y/((double)crnt->nlnd);
      rz+=crnt_cl->nd[ind].z/((double)crnt->nlnd);
    }

    printf("\n");
    printf("m4_wave_speeds: el %d | %d %d | %f %f \n",g_istep,crnt->root,ifc,run->sol_L->phi[0],run->sol_R->phi[0]);
    //printf("normal: %e %e %e \n",crnt_cl->nx[f],crnt_cl->ny[f],crnt_cl->nz[f]);
