...
void m3_deltavalues_V0(struct RUN *run,struct BRANCH *crnt,double * delta_temp,int f_temp) {
  ...
  m3_facevalues_MUSCL(run,crnt,f_temp,0,bc_temp,g_S_cons_L);  // gives value to g_S_cons_L: matrix with conservative values from the center of the cell 
  m3_facevalues_MUSCL(run,crnt,f_temp,1,bc_temp,g_S_cons_R);  // gives value to g_S_cons_R: matrix with conservative values from the neigboring cell or BC
  
  if (PRIMTV==1){
    //m3_cons2primtv(run,run->sol_L);  // vec  to wec
    //m3_cons2primtv(run,run->sol_R); // vec1 to wec1
...
void m3_deltavalues_V1(struct RUN *run,struct BRANCH *crnt,int f,int fo) {
  ...
  if (PRIMTV==1){
    m3_cons2primtv(g_S_cons_R,g_S_prim_R);  // g_S_cons_R  to g_S_prim_R
