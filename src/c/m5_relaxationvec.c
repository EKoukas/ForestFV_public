...
void m5_relaxationvec(struct RUN * run,struct BRANCH * crnt) {
  ...
  double e_elast,e_elastic_K,Ys,are_elast,Gmag;
  ...
  for(iv=0;iv<eqtypn[3];++iv){
    ...
    else if (avf_R[iv]>=(1.0-(AMIN/100.0))) { avf_R[iv]=1.0-(AMIN/100.0); }   
    avfsum  += avf_R[iv]; 
  }  
  ...
  for(iv=0;iv<eqtypn[3];++iv){
    avf_init[iv] = avf_R[iv];
  }
  avf_init[eqtypn[3]] = 1.0 - avfsum;

	i_cons=eqtypi[5];
	Amat[0] = run->Vec_temp_R[0+i_cons];
	Amat[1] = run->Vec_temp_R[1+i_cons];
	Amat[2] = run->Vec_temp_R[2+i_cons];

	Amat[3] = run->Vec_temp_R[3+i_cons];
	Amat[4] = run->Vec_temp_R[4+i_cons];
	Amat[5] = run->Vec_temp_R[5+i_cons];

	Amat[6] = run->Vec_temp_R[6+i_cons];
	Amat[7] = run->Vec_temp_R[7+i_cons];
	Amat[8] = run->Vec_temp_R[8+i_cons];
  ...
  if (PRELAX==1) {
    ...
    for (iv=0;iv<eqtypn[0];++iv){
      ...
      if (MATERMUSH[iv]!=0.0) { are_elast=m5_elastic_energy_solid_V2(Gmat,Gmag,MATERMUSH[iv]); } 
      else {                    are_elast=0.0;                                                 }

      t0_m[iv] = 1.0/(ar_R[iv]/avf_R[iv]);
      p0_m[iv] = ((MATERGAMA[iv]-1.0)*(are_R[iv] - are_elast)/avf_R[iv]) - MATERGAMA[iv]*MATERPINF[iv]; 
    ...
    for (i=0;i<9;i++){ 
      Amat[i] = Amat[i]*(avf_R[0]/avf_init[0]);
    }
    ...
    for(iv=0;iv<NMATERIALS;++iv){
      ...
      if (MATERMUSH[iv]!=0.0) { are_elastic_temp[iv]=m5_elastic_energy_solid_V2(Gmat,Gmag,MATERMUSH[iv]); } 
      else {                    are_elastic_temp[iv]=0.0;                                                 }

      e_elastic_K = are_elastic_temp[iv]/(ar_R[iv]);
      Ys = ar_R[iv]/rho;
      e_elast += Ys*e_elastic_K;
    }
    
    e_hyd = rE_total/rho - 0.5*(pow((u_vel),2.0) + pow((v_vel),2.0) + pow((w_vel),2.0)) - e_elast;
    ...
    for (iv=0;iv<eqtypn[0];++iv){
      are_R[iv] = (avf_R[iv]*(p_relaxed + MATERGAMA[iv]*MATERPINF[iv]) / (MATERGAMA[iv]-1.0)) + are_elastic_temp[iv]; 
    }
    ...
    while (((cond_2>=pow(10.0,-12.0)) || (cond_1>=pow(10.0,-4.0))) && (iter_NR<=101)) {  
      ...
      if ((iter_NR>=100)) { 
        ...
        if (((fabs(pe_old-pe_new)<pow(10.0,3.0))) || (isnan(pe_new)==1)) {
          ...
          printf("avfsum | %e | %e | %e %e %e \n",
          avf_init[0]+avf_init[1]+avf_init[2],
          avf_R[0]+avf_R[1]+avf_R[2],
          avf_init[0],avf_init[1],avf_init[2] ); 
          printf(" \n"); 
    ...
    for (i=0;i<9;i++){ 
      //Amat[i][j] = Amat[i][j]*pow((avf_R[0]/avf_init[0]),(1.0/3.0));
      Amat[i] = Amat[i]*(avf_R[0]/avf_init[0]);
    }
    ...
    if ((isnan(p_relaxed)==1)){
      printf(" \n");
      printf("Relaxation NR: p_relaxed NaN %d %d %d | %e %e %e | %e %e | %e %e  \n",crnt->root,crnt->type,g_istep,e_hyd,num,den,pe,e_elast,avf_R[0],avf_R[1]);
      exit(0);
    }
    ...
    if (irelax==1){
      ...
      for(iv=0;iv<NMATERIALS;++iv){
        if(avf_R[iv]>eps_sml){
          ...
          pkinit[iv] = ((MATERGAMA[iv]-1.0)*(are_R[iv] - are_elast)/avf_R[iv]) - MATERGAMA[iv]*MATERPINF[iv];
    ...
    i_as = 0;
    if ((avf_R[0]<avf_R[1]) && (MATERMUSH[1]!=0.0)) {
      i_as = 1;
    } 
    for (i=0;i<9;i++){ 
      //Amat[i][j] = Amat[i][j]*pow((avf_R[0]/avf_init[0]),(1.0/3.0));
      Amat[i] = Amat[i]*(avf_R[i_as]/avf_init[i_as]);
    }
  
    // adjust specific energy
    e_elast = 0.0;
  ...
  for(iv=0;iv<eqtypn[3];++iv){ 
    ...
    if ((isnan(avf_R[iv])==1)){
      printf(" \n");
      printf("Rlx_v2: FAIL at vf %d %d | %e %e | %e %e | %e | \n",g_istep,crnt->root,avf_R[iv],avf_init[iv],pe,p_relaxed,are_elastic_temp[iv]);
      exit(0);
    }
  ...
  for(iv=0;iv<eqtypn[5];++iv){    
		run->Vec_temp_R[iv+eqtypi[5]] = Amat[iv];
  }
