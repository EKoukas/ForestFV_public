...
int numlfc(int typ){
  if      (typ==0) {return 6;}
  else if (typ==1) {return 4;}
  else if (typ==2) {return 5;}
}
...
int numlnd(int typ){
  ...
  else if (typ==2) {return 6;}
}


int numfcnd(int ifc, int typ){
  if      (typ==0)        {return 4;}
  else if (typ==1)        {return 3;}
  else if (typ==2&&ifc<3) {return 4;}
  else if (typ==2&&ifc>2) {return 3;}
}
...
int fcndrot (int ifcnd, int iangl,int typ) {
  ...
  if (typ==0){
    ...
    M[1][0]=1; M[1][1]=2; M[1][2]=3; M[1][3]=0;
    M[2][0]=2; M[2][1]=3; M[2][2]=0; M[2][3]=1;
    M[3][0]=3; M[3][1]=0; M[3][2]=1; M[3][3]=2;
...
int  oppface (int i, int ifc,int type) {
  ...
  if (type==0){
    ...
    else if (ifc==1){if(i==0){fc=2;};if(i==1){fc=3;};if(i==2){fc=0;};}
    else if (ifc==2){if(i==0){fc=3;};if(i==1){fc=0;};if(i==2){fc=1;};}
    else if (ifc==3){if(i==0){fc=0;};if(i==1){fc=1;};if(i==2){fc=2;};}

  } else if (type==2){

    if      (ifc==0){if(i==0){fc=1;};if(i==1){fc=2;};}
    else if (ifc==1){if(i==0){fc=2;};if(i==1){fc=0;};}
    else if (ifc==2){if(i==0){fc=0;};if(i==1){fc=1;};}
    else if (ifc==3){if(i==0){fc=4;};}
    else if (ifc==4){if(i==0){fc=3;};}
 ...
 int noppface (int ifc,int type) {
  ...
  else if (type==2){ n=1; if(ifc<3){n=2;} }
  return n;
...
int fcrefnd2elnd (int ifc, int ifcnd,int typ) { //reflected fc node to element nd
  ...
  if (typ==0) {
    ...
    M[0][0]=0; M[1][0]=3; M[2][0]=2; M[3][0]=2; M[4][0]=2; M[5][0]=6;
    M[0][3]=1; M[1][3]=7; M[2][3]=6; M[3][3]=0; M[4][3]=3; M[5][3]=4;
    M[0][2]=5; M[1][2]=5; M[2][2]=7; M[3][2]=4; M[4][2]=1; M[5][2]=5;
    M[0][1]=4; M[1][1]=1; M[2][1]=3; M[3][1]=6; M[4][1]=0; M[5][1]=7;

  } else if (typ==1) {

    M[0][0]=1; M[1][0]=0; M[2][0]=1; M[3][0]=2;
    M[0][1]=2; M[1][1]=3; M[2][1]=3; M[3][1]=3;
    M[0][2]=0; M[1][2]=1; M[2][2]=2; M[3][2]=0;
    M[0][3]=4; M[1][3]=5; M[2][3]=6; M[3][3]=7;

    if (ifc<0||ifc>3||ifcnd>3||ifcnd<0){printf ("oups fcrefnd %d %d \n",ifc,ifcnd);}

  } else if (typ==2) {
 
    M[0][0]=0; M[1][0]=2; M[2][0]=0; M[3][0]=0; M[4][0]=3;
    M[0][1]=3; M[1][1]=1; M[2][1]=2; M[3][1]=1; M[4][1]=5;
    M[0][2]=4; M[1][2]=4; M[2][2]=5; M[3][2]=2; M[4][2]=4;
    M[0][3]=1; M[1][3]=5; M[2][3]=3; M[3][3]=6; M[4][3]=7;
