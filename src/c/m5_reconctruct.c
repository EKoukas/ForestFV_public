...
void m5_reconctruct_V0(struct RUN *run,struct BRANCH *crnt,int f,double dist_p1_cf,double dist_p1_cc,
  ...
  for(iv=0;iv<eqtypn[3];++iv){
    ...
    if (PRIMTV==1){ avf[iv] = g_S_prim_rec_L[iv];            }
 
    if (avf[iv]<= AMIN/100.0)              {  avf[iv] =           AMIN/100.0;   } 
    if (avf[iv]>=(1.0-(2.0*(AMIN/100.0)))) {  avf[iv] = 1.0-(2.0*(AMIN/100.0)); }    
    avfsum  += avf[iv];
  ...
  if (PRIMTV==1){
    m5_prim2conc(g_S_prim_rec_L,g_S_cons_rec_L);  // g_S_prim_rec_L to g_S_cons_rec_L
...
void m5_reconctruct_V1(struct RUN *run,struct BRANCH *crnt,int f, int fo) {
  ...
  for(iv=0;iv<eqtypn[3];++iv){
    ...
    if ((isnan(g_S_prim_rec_L[iv])==1)){ printf("NaN reconstruct %d %d exiting \n",iv,eqtypn[3] ); exit(0); }
