...
void m4_exportfield_linked (struct RUN * run) {
  ...
  for (ipt=0;ipt<run->con->size;ipt++){ // Loop partitions
      ...
      for (il=0;il<8*run->topo->pleaves;il++){ // Loop links
        if (LKNN[il]>0){
          ...
          r=0.0; for (isp=0;isp<6;isp++){Y[isp]=0;}   // Zero
    	    for(iv=0;iv<(NEQ+(NMATERIALS+1)+1);iv++){var[iv]=0.0;}
          for (inode=0;inode<LKNN[il];inode++){ // Loop link nodes
            ...
            var[NEQ+NMATERIALS+1] += brch->el->SI[0]/LKNN[il];
            //=========================================================================================                                               
          ...
          for(iv=0;iv<(NEQ+(NMATERIALS+1)+1);iv++){exprt_real[ill*nll+5+iv]=var[iv];}
	...
	//printf("export B %d %d %d %d \n",ipt,run->con->rank,nllinks,ill*NEQ);
        ...
        MPI_Send(exprt_real,ill*(5+(NEQ+(NMATERIALS+1)+1)),MPI_DOUBLE,0,1*nsz+ipt,MPI_COMM_WORLD);
    ...
    if (run->con->rank==0){
      ...
      if (ipt!=0){
        ...
        MPI_Recv(exprt_real,ill*(5+(NEQ+(NMATERIALS+1)+1)),MPI_DOUBLE,ipt,1*nsz+ipt,MPI_COMM_WORLD,&status);
      ...
      for (il=0;il<ill;il++){ // Loop links
        ...
        for(iv=NEQ;iv<(NEQ+(NMATERIALS+1)+1);iv++)  { fprintf(tp," %e ",exprt_real[nll*il+5+iv] ); }
