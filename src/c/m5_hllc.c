...
void m5_hllc(struct RUN *run,struct BRANCH * crnt,int in,int ifc) {
  ...
  m5_wave_speeds(run,crnt,ifc,&u_S,&SL,&SR,&uL,&vL,&wL,&s11L,&s12L,&s13L,
  ...
  if((0.0<=SL) && (flag_end==0)){ // Left state
    ...
    m5_flux(run,run->sol_L,flag_ifs);                                
  }

  if(((SL<=0.0)&&(0.0<=u_S)) && (flag_end==0)) { // Left star state  
    flag_ifs = 1; flag_end = 1;
    m5_star_region(run,crnt,run->sol_L,ifc,flag_ifs,
  ...
  if(((u_S<=0.0)&&(0.0<=SR)) && (flag_end==0)) { // Right star state
    ...
    m5_star_region(run,crnt,run->sol_R,ifc,flag_ifs,
                   u_S,SL,SR,uL,vL,wL,uR,vR,wR,s11L,s12L,s13L,s11R,s12R,s13R);
    m5_flux(run,run->sol_R,flag_ifs); 
  ...
  if (crnt->cl->fc[ifc].bc==1){ // Symmetry
    ...
    m5_facevalues(run,crnt,in,ifc,0,0);
    m5_facevalues(run,crnt,in,ifc,1,1);
