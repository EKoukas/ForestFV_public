...
void m0_residual(struct RUN * run, struct BRANCH * crnt) {
  ...
  for(f=0; f<(crnt->nlfc); f++) {  // Loop for faces  
    ...
    if (flag_compute==1) {
      ...
      ineig=0;
      if      (ORDER==1){ m0_facevalues(run,crnt,ineig,f,0,0); }  // crnt->el->S  to sol_L
      else if (ORDER==2){ m0_facercnstr(run,crnt,ineig,f,0,0); }  // crnt->el->SF to sol_L

      for (ineig=0;ineig<Nneig;++ineig) {
        
        bc_temp = crnt->cl->fc[f].bc;
        if      (ORDER==1){ m0_facevalues(run,crnt,ineig,f,bc_temp,1); }  // sol_R
        else if (ORDER==2){ m0_facercnstr(run,crnt,ineig,f,bc_temp,1); }  // sol_R
        
        if (NS==1) {flux_viscous(run,crnt,run->sol_L,run->sol_R);}
        
        m0_hllc(run,crnt,ineig,f); 
