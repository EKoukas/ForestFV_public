...
void m5_facercnstr(struct RUN *run,struct BRANCH * crnt,int in,int ifc,int bc_temp,int imode){
  ...
  r_sol->r=0.0;
	for(i=0;i<eqtypn[0];++i){
	  r_sol->r     += temp_el->SF[ifc_temp][i];
	  r_sol->ra[i]  = temp_el->SF[ifc_temp][i];
	  r_sol->vec[i] = temp_el->SF[ifc_temp][i];
	}
  ...
  i_cons = eqtypi[1];
	r_sol->u = temp_el->SF[ifc_temp][i_cons + 0] / r_sol->r;
	r_sol->v = temp_el->SF[ifc_temp][i_cons + 1] / r_sol->r;
	r_sol->w = temp_el->SF[ifc_temp][i_cons + 2] / r_sol->r;

  r_sol->vec[i_cons + 0] = temp_el->SF[ifc_temp][i_cons + 0];
  r_sol->vec[i_cons + 1] = temp_el->SF[ifc_temp][i_cons + 1];
  r_sol->vec[i_cons + 2] = temp_el->SF[ifc_temp][i_cons + 2];
  ...
  for(i=0;i<eqtypn[3];++i){ 
    avf_temp[i]   = temp_el->SF[ifc_temp][i_cons + i]; 
  ...
  for(i=0;i<eqtypn[3];++i){
    ...
    if      (avf_c_temp[i]>=(1.0-(AMIN/100.0))) { avf_c_temp[i] = 1.0-(AMIN/100.0); }
    else if (avf_c_temp[i]<=      AMIN/100.0)   { avf_c_temp[i] =      AMIN/100.0;  }
    avfsum += avf_temp[i]; 
  }  
  ...
  i_cons=eqtypi[4];
	for(i=0;i<eqtypn[4];++i){
	  r_sol->are[i]          = temp_el->SF[ifc_temp][i_cons + i];
    r_sol->vec[i_cons + i] = temp_el->SF[ifc_temp][i_cons + i];
	}
  ...
  for (i=0;i<3;++i){
    for (j=0;j<3;++j){
      Amat[i*3+j]            = temp_el->SF[ifc_temp][i_cons + k];
      r_sol->Amat[i*3+j]     = temp_el->SF[ifc_temp][i_cons + k]; 
      r_sol->vec[i_cons + k] = temp_el->SF[ifc_temp][i_cons + k]; 
      k++;
    }
  }
  ...
  for (i=0;i<eqtypn[4];++i) {   
    ...
    press = ((MATERGAMA[i]-1.0)*(r_sol->are[i] - are_elast)/r_sol->avf[i]) - MATERGAMA[i]*MATERPINF[i];  // Stiffened-gas / ideal gas

    //r_sol->c2Y[i]     = (avf_temp[i]/r_sol->r)*(MATERGAMA[i]*(press+MATERPINF[i]) + (4.0/3.0)*MATERMUSH[i]);
    r_sol->c2Y[i]     = (r_sol->avf[i]/r_sol->r)*MATERGAMA[i]*(press+MATERPINF[i]);
