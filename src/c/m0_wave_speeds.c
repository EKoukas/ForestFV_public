...
void m0_wave_speeds(struct RUN *run, struct BRANCH * crnt,int ifc,double* S_S,double* SL, double* SR,
  ...
  *S_S=( (run->sol_L->r*pow((*uL),2.0) + (*s11L)) - (run->sol_R->r*pow((*uR),2.0) + (*s11R)) - ( (*SL)*ruL) + ( (*SR)*ruR)) / 
  ...
  if (isnan(*S_S)==1) {
    printf("m0_wave_speeds: S_S nan | %d | %d | %d | %e | %e %e | %e %e | %e %e | %e %e | %e %e \n",run->con->rank,crnt->root,crnt->cl->fc[ifc].bc,*S_S,*SL,*SR,cL,cR,(*uL),(*uR),(*s11L),(*s11R),run->sol_L->r,run->sol_R->r);
