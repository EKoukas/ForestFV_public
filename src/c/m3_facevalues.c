...
void m3_facevalues(struct RUN *run,struct BRANCH * crnt,int in,int ifc,int bc_temp,int imode) {
  ...
  struct SOL * r_sol;
  struct LEAF * temp_el;
  double press,avfsum,avf_dif,avf_per,e_hyd;
  double num,den,rho;
  double avf_temp[10];
  ...
  for(v=0;v<eqtypn[3];++v){
    ...
    else if (avf_temp[v]  >=(1.0-(AMIN/100.0))) { avf_temp[v]   = 1.0-(AMIN/100.0); }  
     
    if (avf_c_temp[v]>=(1.0-(AMIN/100.0))) { avf_c_temp[v] = 1.0-(AMIN/100.0); }
    else if (avf_c_temp[v]<=      AMIN/100.0)   { avf_c_temp[v] =      AMIN/100.0;  }
    avfsum   += avf_temp[v]; 
  }  
  ...
  for (v=0;v<eqtypn[4];++v) {    
    rho +=  r_sol->ra[iv];
    num += (r_sol->avf[iv] * MATERGAMA[iv] * MATERPINF[iv]) / (MATERGAMA[iv]-1.0);
    den +=  r_sol->avf[iv]                                  / (MATERGAMA[iv]-1.0);
  }
  press = (rho*e_hyd - num) / den;
