...
void m1_residual(struct RUN * run, struct BRANCH * crnt) {
  ...
  for(f=0; f<(crnt->nlfc); f++) {  // Loop for faces  
    ...
    if (flag_compute==1) {
      ...
      ineig=0;
      if      (ORDER==1){ m1_facevalues(run,crnt,ineig,f,0,0); }  // sol
      else if (ORDER==2){ m1_facercnstr(run,crnt,ineig,f,0,0); }  // sol

      for (ineig=0;ineig<Nneig;++ineig) {
  
        if (ORDER==1){
          if(crnt->cl->fc[f].bc==0){ 
            m1_facevalues(run,crnt,ineig,f,1,1);
          } else {
            m1_facevalues(run,crnt,ineig,f,0,1);
            m1_bc(crnt,crnt->cl->fc[f].bc,f,run);
          }
        } else if (ORDER==2){
          if(crnt->cl->fc[f].bc==0){
            m1_facercnstr(run,crnt,ineig,f,1,1);  // sol1
          } else {
            m1_facercnstr(run,crnt,ineig,f,0,1);  // sol1
            m1_bc(crnt,crnt->cl->fc[f].bc,f,run);    // sol1
          }
        }
        
        if (NS==1) {flux_viscous(run,crnt,run->sol_L,run->sol_R);}
        
        m1_hllc(run,crnt,ineig,f); 
